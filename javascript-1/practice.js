...
let doubled = myNumbers.map((number, i, arr) => {
  return number * 2;
});
...
let filtered = myNumbers.filter((number, i, arr) => {
  return number > 100;
});
...
let total = myNumbers.reduce((acc, num, i, arr) => {
  return acc + num;
}, 0);
...
let forTheLoveOfGeorge = notGeorge.map((name, i, arr) => {
  return (name = "George");
});
...
let enemies = people.filter((person, i, arr) => {
...
let totallyAwesome = people.reduce((acc, person, i, arr) => {
  console.log(person.awesomeLevel);
